...
//LOGIN · Método para comprobar datos compositor
export const composerLogin = (req, res) => {
  //Si nos viene el email comprobamos que no exista
  if (req.body.email && req.body.password) {
    ...
    for (let property in req.body) { // Recorrer las propiedades del cuerpo de la solicitud
      ...
      if (property === 'password') { // Si la propiedad es 'password'
        map.set(property, req.body[property]); // Almacenar el valor de la propiedad en el objeto Map
      }
    }

    selectFrom('compositor', map) // Verificar si las credenciales están bien
      .then(resultado => {
        if (!resultado) { // Si son correctas las credenciales
          res.json(`El Compositor con su contraseña es correcta`); // Enviar una respuesta de éxito al cliente
        } else {
          res.json("El Compositor con su contraseña NO es correcta"); // Enviar una respuesta de error al cliente
        }
      })
      .catch(error => {
        console.error(error); // Imprimir en la consola el error ocurrido al ejecutar la consulta
    ...
    res.json(`Introduce en los Headers los parámetros email y contraseña`); // Enviar una respuesta de éxito al cliente
  }
};
...
// Recibe el nombre de la tabla y un Map con los filtros a aplicar
function selectFrom(tabla, map) { // Función que toma una tabla y un mapa con valores de búsqueda y devuelve una promesa con el resultado de una SELECT
  var query = "SELECT * FROM " + tabla + " WHERE "; // Se arma la consulta SELECT
  var index = map.size; // Obtiene la cantidad de valores de búsqueda en el mapa
  map.forEach(function (value, key) { // Recorre el mapa y agrega cada valor de búsqueda a la consulta
    query += key + " = " + "'" + value + "' " // Agrega la clave y el valor a la cadena de consulta
    if (index > 1) { // Si quedan más valores de búsqueda, agrega un operador AND
      query += 'AND '
    };
    index--;
  });
  return pool.query(query) // Se ejecuta la consulta SELECT y se retorna una promesa con el resultado
    .then(rows => {
      if (rows[0][0] == null) {// Si la consulta no encontró resultados, se retorna true
        return true;
      } else {// Si la consulta encontró resultados, se retorna false
        return false;
      }
    })
    .catch(err => {
